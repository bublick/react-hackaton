...
const MainPage = ({ crumbs }) => {
    const [technologies, setTechnologies] = useState([])
    const [photos, SetPhotos] = useState([])
    useEffect(
        () => api.users.fetchAllTechs().then((data) => setTechnologies(data)),
        []
    )
    useEffect(
        () => api.users.fetchAllPhotos().then((data) => SetPhotos(data)),
        []
    )
        ...
        <>
            ...
            <ul className='peoples'>
                {photos.length &&
                    photos.map((photo) => (
                        <li
                            onClick={handleChangeSelectImage}
                            className='peoples__item'
                            key={photo}
                        >
                            <img
                                className='peoples__image'
                                src={photo}
                                alt=''
                            />
                        </li>
